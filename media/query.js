...
(function () {
    ...
    const handleResponse = (res) => {
        ...
        const html = result.map((result, index) => {
            const { source, code, explain } = result;

            const codeRes = hljs.highlight(code, { language: 'cpp' }).value;

            return `<div class="item-wrap">
                        <div class="title"><div class="txt">${source}</div> <span class="apply" data-index="${index}">应用</span></div>
                        <div class="content code language-cpp"><pre>${codeRes}</pre></div>
                        <div class="bottom"><pre>${explain}</pre></div>
                        </div>`;
        }).join('');
        ...
        document.getElementById('result').addEventListener('click', function (ev) {
            ...
            if (target.className.toLowerCase() === 'apply') {
                ...
                const text = result[index]?.code; // 假设结果数组为 results
    ...
    btnSearch.addEventListener('click', () => {
        ...
        loading = true;
        const curHtml = btnSearch.innerHTML;
